...
async function searchBggThings() {
	...
	const { resources: api_resources, errors } = BggThingsSearch.create(
		JSON.parse(await bgg_response.json()),
	);
	...
	const db_response = await fetch(
		`${import.meta.env.BASE_API_URL}/resource?${new URLSearchParams({ ids })}`,
	);
	if (!db_response.ok) {
		toast.custom(AlertError, {
			componentProps: {
				error: `${db_response.status} ${
					db_response.statusText
				}: ${await db_response.text()}`,
			},
		});
	}
	const db_resources = JSON.parse(await db_response.json()).map(
		Resource.create,
	);
	const bgg_to_db_id = db_resources.reduce(
		(acc, resource) => acc.set(resource.bgg_id, resource.id),
		new Map(),
	);
