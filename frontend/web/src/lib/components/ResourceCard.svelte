...
<div class="flex flex-col gap-3 justify-end w-[200px] h-[300px]">
  ...
  <div class="flex justify-start gap-2 items-center h-[10%]">
    <div class="flex text-white">
      ...
      <Button on:click={toggleTrackResource} size="icon" class="group rounded-r-none w-8 h-8 hover:bg-secondary {tracked ? 'text-secondary' : ''}">
        <Bookmark fill={tracked ? "var(--secondary)" : "none"} class="group-hover:bg-secondary rounded-sm group-hover:text-white" />
      </Button>
