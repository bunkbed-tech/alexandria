...
use models::{AlexandriaResource, BoardGame, ResourceMeta};

use crate::{http::respond, state::AppState, services::models::Trackable};
...
pub async fn list_resources(pool: &PgPool, resources: Option<Vec<AlexandriaResource>>) -> Result<Vec<AlexandriaResource>, String> {
...
async fn track_resource(pool: &PgPool, resource: AlexandriaResource) -> Result<AlexandriaResource, String> {
    ...
    let db_resource = match resource {
        AlexandriaResource::AnimeMovie(movie) => AlexandriaResource::AnimeMovie(movie.track(&db_resource_meta)?),
        AlexandriaResource::AnimeTVShow(show) => AlexandriaResource::AnimeTVShow(show.track(&db_resource_meta)?),
        AlexandriaResource::Manga(manga) => AlexandriaResource::Manga(manga.track(&db_resource_meta)?),
        AlexandriaResource::LightNovel(novel) => AlexandriaResource::LightNovel(novel.track(&db_resource_meta)?),
        AlexandriaResource::VideoGame(game) => AlexandriaResource::VideoGame(game.track(&db_resource_meta)?),
        AlexandriaResource::BoardGame(game) => AlexandriaResource::BoardGame(game.track(&db_resource_meta)?),
        AlexandriaResource::VisualNovel(novel) => AlexandriaResource::VisualNovel(novel.track(&db_resource_meta)?),
    };
    Ok(db_resource)
}
...
mod tests {
    ...
    async fn test_list_resources_all(pool: PgPool) {
        ...
        let expected_resources = vec![
            AlexandriaResource::BoardGame(BoardGame {
                id: Some(1),
                meta: ResourceMeta {
                    id: Some(1),
    ...
    async fn test_list_resources_some(pool: PgPool) {
        ...
        let resources = list_resources(&pool, ids).await.unwrap();
        let expected_resources = vec![AlexandriaResource::BoardGame(BoardGame {
            id: Some(1),
            meta: ResourceMeta {
                id: Some(1),
                title: String::from("Scythe"),
    ...
    async fn test_track_resource_untracked(pool: PgPool) {
        let untracked_resource = AlexandriaResource::BoardGame(BoardGame {
            id: None,
            meta: ResourceMeta {
                id: None,
                title: String::from("Scythe"),
        ...
        let expected_tracked_resource = AlexandriaResource::BoardGame(BoardGame {
            id: Some(1),
            meta: ResourceMeta {
                id: Some(1),
                title: String::from("Scythe"),
    ...
    async fn test_track_resource_tracked(pool: PgPool) {
        let tracked_resource = AlexandriaResource::BoardGame(BoardGame {
            id: Some(1),
            meta: ResourceMeta {
                id: Some(1),
                title: String::from("Scythe"),
    ...
    async fn test_untrack_resource(pool: PgPool) {
        ...
        let untracked_resource = untrack_resource(&pool, tracked_resource.id.unwrap())
            .await
            .unwrap();
        let expected_untracked_resource = AlexandriaResource::BoardGame(BoardGame {
            id: None,
            meta: ResourceMeta {
                id: None,
                title: String::from("Scythe"),
